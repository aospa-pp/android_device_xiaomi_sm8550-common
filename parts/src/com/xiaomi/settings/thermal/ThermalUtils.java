...
public final class ThermalUtils {
    ...
    private int getDefaultStateForPackage(String packageName) {
        for (String i : NAVIGATION_PACKAGES) {
            if (i.equals(packageName))
                return STATE_NAVIGATION;
        }

        for (String i : STREAMING_PACKAGES) {
            if (i.equals(packageName))
    ...
    private boolean isDialerApp(String packageName) {
        final Intent dialingIntent =
                new Intent(Intent.ACTION_DIAL).addCategory(Intent.CATEGORY_DEFAULT);

        final List<ResolveInfo> list = mContext.getPackageManager().queryIntentActivitiesAsUser(
            dialingIntent, PackageManager.MATCH_ALL, UserHandle.myUserId());

        return packageName.equals(list.get(0).activityInfo.packageName);
    }
}
